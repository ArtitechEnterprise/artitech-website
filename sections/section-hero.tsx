  ...
  return (
    <Container className="w-full py-8">
      <div className="relative overflow-hidden">
        <AnimatePresence mode="popLayout" key={hero}>
          <SvgWithImage imageUrl={data.image} className="relative">
              ...
              <div className="absolute left-10 top-1/2 h-[66%] w-[3px] -translate-y-1/2 transform rounded-sm">
                {heroData.map((_, index) => (
                  <span
                    ...
                    key={index}
                    className={`absolute left-1/2 flex h-[60px] w-[60px] -translate-x-1/2 -translate-y-1/2 transform cursor-pointer items-center justify-center rounded-full border-2 border-white hover:bg-white hover:text-black ${index === hero ? "bg-white text-black" : "text-white"} text-xl font-semibold ${index === 0 ? "" : index === 1 ? "top-1/2" : "top-[calc(100%)]"}`}
                  >
                    {index + 1}
                  </span>
                ))}

                <span className="absolute left-1/2 top-[30px] h-[36%] w-[3px] -translate-x-1/2 transform bg-white"></span>

                <span className="absolute left-1/2 top-[calc(57%)] h-[36%] w-[3px] -translate-x-1/2 transform bg-white"></span>
              </div>
            </div>
          </SvgWithImage>
        </AnimatePresence>
